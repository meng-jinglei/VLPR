...
image.font_load(image.UTF8, 16, 16, 0xA00000)  # Load font from Flash address 0xA00000
...
kpu = KPU()
kpu.load_kmodel(0x300000)  # Load model from Flash address 0x300000
...
lp_recog_kpu = KPU()
lp_recog_kpu.load_kmodel(0x500000)  # Load model from Flash address 0x500000
lp_recog_kpu.lp_recog_load_weight_data(0x700000, 1498500)  # Load weight data from Flash address 0x700000
...
while True:
    ...
    if len(dect) > 0:
        for l in dect:
            ...
            lp_resize.replace(vflip=0, hmirror=1)
            lp_resize.pix_to_ai()
            ...
            print(show_lp_str)
            img.draw_string(l[0], l[1] - 18, show_lp_str, color=(255, 128, 0), scale=1)
    img.draw_string(10, 0, "%2.1ffps" % fps, color=(255, 255, 0), scale=1)
    lcd.display(img)
    gc.collect()
